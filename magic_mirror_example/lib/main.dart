  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      onGenerateRoute: (RouteSettings setting) {
        ...
        Widget page =
            MagicMirror.newInstanceS(key, params: arg, canThrowException: true);
        setting = RouteSettings(
            name: uriInfo.key,
            arguments: RouteArguments(uriInfo.uriParams, arg));
        return MaterialPageRoute(
            settings: setting,
            builder: (context) => page ?? Page404(setting: setting));
      },
...
class RouteArguments {
  ...
  RouteArguments(this.uriArgs, this.args);
}

class Page404 extends StatelessWidget {
  final RouteSettings setting;

  const Page404({Key key, @required this.setting}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    var arg = setting.arguments;
    var args = arg is RouteArguments
        ? 'uriParams: ${arg.uriArgs.toString()} \nother:${arg.args.toString()} '
        : arg.toString();
    return Scaffold(
      appBar: AppBar(
